...
def _build_leaderboard() -> str | None:
    """Build the detection leaderboard table with per-segment recall and F1."""
    if not PREDICTIONS_PATH.exists() or not DATASET_PATH.exists():
        return None
...
def _build_comparison_charts(report_data: dict) -> str | None:
    ...
    for key, (title, group_col) in sections_by_key.items():
        ...
        chart_idx += 1
        pj = grouped_bar_plotly(df, metric, group_col)
        blocks.append(
...
def generate_report(output_path: Path = None, open_browser: bool = True) -> Path:
    ...
    if not dataset.empty:
        ...
        sections_html.append(render_static_section(
            ...
            overview_html + leaderboard_html,
        ))
...
<title>AdvPIIBench \u2014 Evaluation Report</title>
{PLOTLY_CDN}
<style>{CSS}</style>
</head>
<body>
<div class="topbar">
    <span class="topbar-title">AdvPIIBench Report</span>
...
{"".join(sections_html)}
<div class="footer">AdvPIIBench \u2014 Evaluation Dashboard</div>
<script>{JS}</script>
</body>
</html>"""
